...
VALUES
    ('MNV1001', N'Nguyễn', N'An', '20-05-1990', '01-01-2015', N'234 Kim Mã', 'PX001', '0123123123', 8000000, 1000000),
    ('MNV1002', N'Trần', N'Bình', '20-08-1992', '15-03-2016', N'567 Láng Hạ', 'PX003', '0234234234', 8000000, 1500000),
    ('MNV1003', N'Lê', N'Cường', '30-12-1988', '01-06-2014', N'890 Đội Cấn', 'PX004', '0345345345', 10000000, 2000000),
    ...
    ('MNV1007', N'Vũ', N'Giang', '20-02-1994', '15-01-2019', N'321 Đống Đa', 'PX008', '0789789789', 800000, 700000),
    ('MNV1008', N'Mai', N'Hương', '20-09-1989', '01-11-2015', N'654 Cầu Giấy', 'PX009', '0890890890', 8000000, 2500000),
    ('MNV1009', N'Bùi', N'Linh', '12-04-1996', '01-03-2020', N'987 Tây Hồ', 'PX010', '0901901901', 800000, 900000),
    ('MNV1010', N'Đỗ', N'Minh', '25-06-1992', '15-05-2017', N'147 Long Biên', 'PX002', '0912912912', 8000000, 1600000);
...
-- 2. Mã hàng, tên hàng và số lượng hàng hiện có của mỗi công ty
	SELECT MH.MAHANG, (MH.SOLUONG - SUM(CTDH.SOLUONG)) AS "Số lượng hàng hiện có"
	FROM MATHANG AS MH 
	JOIN CHITIETDATHANG AS CTDH ON CTDH.MAHANG = MH.MAHANG
	GROUP BY MH.MAHANG, MH.SOLUONG
...
-- 11. Số tiền lương mà mỗi công ty phải trả cho nhân viên
	-- Cách 1
	UPDATE NHANVIEN
	SET PHUCAP = 0
	WHERE PHUCAP IS NULL
...
-- 16. Những mặt hàng nào chưa được khách hàng đặt mua
	SELECT *
	FROM MATHANG
	WHERE MAHANG IN (SELECT MAHANG
					 FROM MATHANG AS MH
...
---1. Tạo thủ tục lưu trữ để thông qua thủ tục này có thể bổ sung thêm một bản ghi mới cho bảng MATHANG 
	...
	BEGIN
			...
			BEGIN
				PRINT N'Thêm không thành công: MAHANG đã tồn tại.';
				RETURN;
			END

			IF NOT EXISTS (SELECT MACONGTY FROM NHACUNGCAP   WHERE MACONGTY = @MACONGTY)
			BEGIN
				PRINT N'Thêm không thành công: MACONGTY không tồn tại.';
				RETURN;
			END

			IF NOT EXISTS (SELECT MALOAIHANG FROM LOAIHANG WHERE MALOAIHANG = @MALOAIHANG)
			BEGIN
				PRINT N'Thêm không thành công: MALOAIHANG không tồn tại.';
				RETURN;
			END
			INSERT INTO MATHANG (MAHANG, TENHANG, MACONGTY, MALOAIHANG, SOLUONG, DONVITINH, GIAHANG) 
	...
	-- Sử dụng thủ tục đã được xây dựng
	EXEC INSERT_MATHANG 
		@MAHANG = 'MH1015',
		@TENHANG = N'Mặt hàng B',
		@MACONGTY = 'NCC001',
		@MALOAIHANG = 'MLH1002',
		@SOLUONG = 50,
		@DONVITINH = 'Hộp',
...
--2. Tạo thủ tục lưu trữ có chức năng thống kê tổng số lượng hàng bán được của một mặt hàng có mã bất kỳ (mã mặt hàng cần thống kê là tham số của thủ tục).
	CREATE PROCEDURE PR_ThongKeBanHang 
		   @MAHANG CHAR(7)
	AS
	BEGIN
		select  c.MAHANG,sum(c.SOLUONG) SoLuongBan
		from  DONDATHANG as d, CHITIETDATHANG as c
		where  d.SOHOADON=c.SOHOADON and c.MAHANG=@MAHANG
		group by c.MAHANG
	END

	-- Sử dụng tục đã xây dựng
	EXEC PR_ThongKeBanHang'MMH1001'
...
-- 3. Viết hàm trả về một bảng trong đó cho biết tổng số lượng hàng bán được của mỗi mặt hàng.
	...
	CREATE FUNCTION FN_ThongKe()
	RETURNS TABLE 
	AS
	RETURN
		(
		SELECT  m.MAHANG,sum(c.SOLUONG) SoLuongBan,sum(m.SOLUONG) as SoLuongCo
		FROM  DONDATHANG as d, CHITIETDATHANG as c, MATHANG as m 
		WHERE  d.SOHOADON=c.SOHOADON  and c.MAHANG= m.MAHANG
		GROUP BY m.MAHANG
		) 

	--Sử dụng hàm đã xây dựng
	SELECT * FROM FN_ThongKe()
	GO


--4. Viết trigger cho bảng CHITIETDATHANG theo yêu cầu sau: 
	-- • Khi một bản ghi mới được bổ sung vào bảng này thì giảm số lượng hàng hiện có nếu số lượng hàng hiện có 
		-- lớn hơn hoặc bằng số lượng hàng được bán ra. Ngược lại thì huỷ bỏ thao tác bổ sung. 

	CREATE TRIGGER tgInsert_CHITIETDATHANG
	ON CHITIETDATHANG
	FOR INSERT
	AS
	BEGIN 
		IF EXISTS (SELECT 1 FROM MATHANG mh, inserted i WHERE i.MAHANG = mh.MAHANG and i.SOLUONG > mh.soluong)
		BEGIN
			PRINT 'Số lượng hàng không đủ';
			SELECT TENHANG, mh.SOLUONG
			FROM MATHANG mh join inserted i ON i.MAHANG = mh.MAHANG
			ROLLBACK;
			RETURN;
		END
		UPDATE MATHANG
		...
		FROM inserted i
		WHERE i.MAHANG = MATHANG.MAHANG 

		-- Tự động lấy giá sản phẩm ở bảng MATHANG nhân 1.5 lần
		UPDATE CHITIETDATHANG 
		SET GIABAN = mh.GIAHANG * 1.5
		FROM inserted i, MATHANG MH
		WHERE CHITIETDATHANG.MAHANG = i.MAHANG AND MH.MAHANG = i.MAHANG
	END
	GO

	-- Insert thử vào bảng CHITIETDONHANG 1 đơn hàng có mã hoá đơn là MHD1001 và mua mã hàng MMH1001
	INSERT INTO CHITIETDATHANG (SOHOADON, MAHANG, SOLUONG, MUCGIAMGIA) 
	VALUES
		('MHD1001', 'MMH1002',10, 5)
...
-- • Khi cập nhật lại số lượng hàng được bán, kiểm tra số lượng hàng được cập nhật lại có phù hợp hay không 
	...
	BEGIN
		IF EXISTS(SELECT sohoadon FROM inserted WHERE soluong  <0)
			BEGIN
				PRINT N'Hàng mua không được nhỏ hơn 0';
		...
		ELSE
			BEGIN
				...
				IF EXISTS(SELECT mahang FROM mathang
					WHERE soluong<0)
					BEGIN
						PRINT N'Số lượng hàng không đủ';
						ROLLBACK TRANSACTION;
						RETURN;
					END
			END
	...
	-- Update lại bảng Chi tiết đặt hàng ở hoá đơn MHD1001 Và mã hàng MMH1001 
	UPDATE CHITIETDATHANG
	SET SOLUONG = 10
	WHERE SOHOADON = 'MHD1001' and MAHANG ='MMH1001'
	SELECT * FROM CHITIETDATHANG
	SELECT * FROM MATHANG
